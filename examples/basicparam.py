...
class ParamExample:
    ...
    def _param_callback(self, name, value):
        ...
        if len(self._param_check_list) == 0:
            ...
            # Create a new random value [0.00,1.00] for pid_attitude.pitch_ki
            # and set it
            pkd = random.random()
            print('')
            print('Write: pid_attitude.pitch_ki={:.2f}'.format(pkd))
            self._cf.param.add_update_callback(group='pid_attitude',
                                               name='pitch_ki',
                                               cb=self._a_pitch_ki_callback)
            # When setting a value the parameter is automatically read back
            # and the registered callbacks will get the updated value
            self._cf.param.set_value('pid_attitude.pitch_ki',
                                     '{:.2f}'.format(pkd))

    def _a_pitch_ki_callback(self, name, value):
