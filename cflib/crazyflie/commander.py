...
class Commander():
    ...
    def send_base_pose(self, index, w, x, y, z, theta, thrust):

        pk = CRTPPacket()
        pk.port = CRTPPort.COMMANDER_GENERIC
        pk.data = struct.pack('<Bfffffff', TYPE_BASE,
                              index, w, x, y, z, theta, thrust)
        self._cf.send_packet(pk)


    def send_thrust(self, thrust):
        """
        send thrust only. for testing
        """
        pk = CRTPPacket()
        pk.port = CRTPPort.COMMANDER_GENERIC
        pk.data = struct.pack('<Bf', TYPE_THRUST, thrust)
        self._cf.send_packet(pk)


    def send_twod(self, index:int, w:float, x:float, y:float, z:float, alpha:float, beta: float, thrust: float):
        """
        send commands for two-DOF joint multi-qc platform
        """
        pk = CRTPPacket()
        pk.port = CRTPPort.COMMANDER_GENERIC
        pk.data = struct.pack('<BBfffffff', TYPE_TWOD, index, w, x, y, z, alpha, beta, thrust)
        self._cf.send_packet(pk)

    def send_omni(self, compQuat, wx_r, wy_r, wz_r, thrust):
        """
        send commands 
        """
        pk = CRTPPacket()
