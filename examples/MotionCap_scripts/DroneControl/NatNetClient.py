...
class NatNetClient:
    def __init__( self ):
        ...
        self.serverIPAddress = "10.1.13.27" 

        # Change this value to the IP address of your local network interface
        self.localIPAddress = "10.1.13.69"
    ...
    def __unpackMocapData( self, data ):
        ...
        if( ( self.__natNetStreamVersion[0] >= 3 ) or  major == 0 ):
            stampCameraExposure = struct.unpack('I',data[offset:offset+4])[0]
            # stampCameraExposure = int.from_bytes( data[offset:offset+8], byteorder='little' )
            offset += 8
            stampDataReceived = struct.unpack('I',data[offset:offset+4])[0]
            # stampDataReceived = int.from_bytes( data[offset:offset+8], byteorder='little' )
            offset += 8
            stampTransmit = struct.unpack('I',data[offset:offset+4])[0]
